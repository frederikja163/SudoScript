...
internal static class Program
{
    ...
    {
        // TODO: This is temporary, to test, develop and debug the solver.
        List<IRule> oneRule = new List<IRule>() { new OneRule() };
        ...
        Board board = new Board(allCells, units);
        Solver.Solve(board);

        Unit CreateRow(int y)
        {
            ...
            return new Unit(cells, oneRule);
        }

        Unit CreateColumn(int x)
        {
            List<CellReference> cells = new List<CellReference>();
            for (int y = 1; y <= 9; y++)
            {
