...
public static class SolverMultiThread
{
    ...
    private static HashSet<Board> SolveRecAll(Board board, int solutionCount, int limit, bool random)
    {
        ...
        ConcurrentBag<Board> solutionsBag = new ConcurrentBag<Board>();
        Parallel.ForEach(cell.Candidates(), candidate =>
        {
            // Return if the limit of solutions has been reached

            if ((limit != 0) && ((solutionsBag.Count() + solutionCount) >= limit))
            {
                return;
            }
            ...
            // Call solve on the new board.
            HashSet<Board> subSolutions = SolveRecAll(clonedBoard, (solutionCount + solutionsBag.Count()), limit, random);
            foreach (Board subSolution in subSolutions)
            {
                solutionsBag.Add(subSolution);
            }
        });

        solutions.UnionWith(solutionsBag);
        return solutions;
    }
