...
internal sealed class SolverTests
{
    ...
    public void FindSolutionsTwoUniqueCellsTest()
    {
        ...
        List<Board>? boardList = SolverDynamicCandidatesByUnits.FindSolutions(board);

        Assert.IsNotNull(boardList);
        Assert.That(boardList.Count(), Is.EqualTo(72));

        List<Board>? randomBoardList = SolverDynamicCandidatesByUnits.FindSolutions(board, 0, true);
